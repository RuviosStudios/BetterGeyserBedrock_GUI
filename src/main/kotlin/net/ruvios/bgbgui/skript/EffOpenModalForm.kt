...
class EffOpenModalForm : Effect() {
    ...
    override fun init(
        ...
        when (matchedPattern) {
            0 -> {
                ...
                yesButton = exprs[2] as Expression<String>
                noButton = exprs[3] as Expression<String>
            ...
            2 -> {
                ...
                yesButton = exprs[3] as Expression<String>
                noButton = exprs[4] as Expression<String>
                player = exprs[5] as Expression<Player>
            }
            else -> {
                id = exprs[0] as Expression<String>
                title = exprs[1] as Expression<String>
    ...
    override fun toString(event: Event?, debug: Boolean): String {
        return "open modal form titled ${title.toString(event, debug)} to ${player.toString(event, debug)}"
    }

    companion object {
        fun register() {
            Skript.registerEffect(
                EffOpenModalForm::class.java,
                "open modal form titled %string% with description %string% with yes %string% and no %string% to %player%",
                "open modal form titled %string% with description %string% to %player%",
                "open modal form with id %string% titled %string% with description %string% with yes %string% and no %string% to %player%",
                "open modal form with id %string% titled %string% with description %string% to %player%",
            )
        }
    }
}
